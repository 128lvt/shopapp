...
ALTER TABLE
    social_accounts AUTO_INCREMENT = 10000;
...
ALTER TABLE
    categories AUTO_INCREMENT = 10000;
...
CREATE TABLE products (
    ...
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    category_id INT,
    CONSTRAINT fk_products_categories FOREIGN KEY (category_id) REFERENCES categories (id) ON DELETE
    SET
        NULL
);

ALTER TABLE
    products AUTO_INCREMENT = 10000;
...
ALTER TABLE
    product_variants AUTO_INCREMENT = 10000;
...
ALTER TABLE
    product_images AUTO_INCREMENT = 10000;
...
CREATE TABLE orders (
    ...
    `status` ENUM(
        ...
        'Đã giao hàng',
        'Đã hủy'
    ) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT 'Chờ xác nhận',
    total_money DECIMAL(16, 2) CHECK (total_money >= 0),
...
ALTER TABLE
    order_details AUTO_INCREMENT = 10000;

DELIMITER / / CREATE TRIGGER after_insert_order_details
AFTER
INSERT
    ON order_details FOR EACH ROW BEGIN DECLARE current_stock INT;

-- Kiểm tra nếu variant_id không null
IF NEW.variant_id IS NOT NULL THEN -- Lấy số lượng tồn kho hiện tại
SELECT
...
IF current_stock < NEW.number_of_products THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = 'Insufficient stock available';

ELSE -- Cập nhật tồn kho
UPDATE
    product_variants
SET
    stock = stock - NEW.number_of_products
WHERE
    id = NEW.variant_id;
...
CREATE EVENT IF NOT EXISTS delete_expired_tokens ON SCHEDULE EVERY 5 MINUTE DO
DELETE FROM
    tokens
WHERE
    expiration_date < NOW();

-- Thiết lập timezone và event scheduler
SET
...
ALTER EVENT delete_expired_tokens ENABLE;
