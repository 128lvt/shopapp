...
DO DELETE FROM `tokens` WHERE `expiration_date` < NOW();

-- View thống kê
CREATE OR REPLACE VIEW `top_selling_products_monthly` AS
SELECT
    YEAR(`o`.`order_date`) AS `year`,
    SUM(IF(MONTH(`o`.`order_date`) = 1, `od`.`number_of_products`, 0)) AS January,
    SUM(IF(MONTH(`o`.`order_date`) = 2, `od`.`number_of_products`, 0)) AS February,
    SUM(IF(MONTH(`o`.`order_date`) = 3, `od`.`number_of_products`, 0)) AS March,
    SUM(IF(MONTH(`o`.`order_date`) = 4, `od`.`number_of_products`, 0)) AS April,
    SUM(IF(MONTH(`o`.`order_date`) = 5, `od`.`number_of_products`, 0)) AS May,
    SUM(IF(MONTH(`o`.`order_date`) = 6, `od`.`number_of_products`, 0)) AS June,
    SUM(IF(MONTH(`o`.`order_date`) = 7, `od`.`number_of_products`, 0)) AS July,
    SUM(IF(MONTH(`o`.`order_date`) = 8, `od`.`number_of_products`, 0)) AS August,
    SUM(IF(MONTH(`o`.`order_date`) = 9, `od`.`number_of_products`, 0)) AS September,
    SUM(IF(MONTH(`o`.`order_date`) = 10, `od`.`number_of_products`, 0)) AS October,
    SUM(IF(MONTH(`o`.`order_date`) = 11, `od`.`number_of_products`, 0)) AS November,
    SUM(IF(MONTH(`o`.`order_date`) = 12, `od`.`number_of_products`, 0)) AS December
